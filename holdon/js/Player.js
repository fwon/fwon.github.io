...
export default class Player extends Point {
    ...
    boom(x, y, color, size) {
        ...
        for (let i = 0; i < self.particleCount; i++) {
            self.particles.push(new Particle({x, y, color, size}));
        }   
    }
    ...
    renderShield() {
        ...
        map.ctx.arc(this.x, this.y, this.shieldRadius + 2, 0, Math.PI*2, false);
        map.ctx.stroke();
    ...
    renderAddScore() {
        ...
        map.ctx.fillStyle = "rgba(255,255,255,"+ this.addScoreText.opacity +")";
        map.ctx.fillText("+" + REDSCORE, this.addScoreText.x + 40, this.addScoreText.y - 30);
